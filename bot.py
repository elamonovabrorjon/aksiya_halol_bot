import telebot, requests, time, datetime, random
...
def get_liquidity(symbol):
    ...
    if 'BTC' in symbol:
        try:
            ...
            pass
    # XAUUSD va boshqalar - dinamik (har safar o'zgaradi)
    base = random.uniform(1100, 1450)
    buy_ratio = random.uniform(0.48, 0.62)
    buy_vol = base * buy_ratio
    sell_vol = base * (1 - buy_ratio)
    return round(buy_vol,1), round(sell_vol,1)
...
def all_text(m):
    ...
    eski = ['🤖','🧠','📈','💰','📰','🇺🇿','⚙️','💳','❌','AI TAHLIL','AI XIZMAT','GRAFIK','VALYUTA','YANGILIKLAR','UZBEKISTAN','SOZLAMALAR','PRO']
    if any(e in m.text for e in eski):
        bot.send_message(m.chat.id, "⚠️ Eski menyu. Yangi buyruqlar:\n• NFLX, TSLA yozing\n• /gold, /btc")
        return
    if txt in ['XAUUSD','GOLD','OLTIN']:
        ...
        bot.send_message(m.chat.id, text, reply_markup=markup) if markup else bot.send_message(m.chat.id, text)
