...
UZSE_STOCKS = {
    ...
    "UZKIMYO":      {"nom": "O'zkimyo farm",          "holat": "🟢 HALOL",    "sabab": "Kimyo va tibbiyot mahsulotlari ishlab chiqarish."},
    "KVARZ":        {"nom": "Kvarts AJ",              "holat": "🟢 HALOL",    "sabab": "Oyna va shisha mahsulotlari ishlab chiqarish. Halol."},
    "UZPHARMA":     {"nom": "UzPharmaInvestment",     "holat": "🟢 HALOL",    "sabab": "Dori-darmon va farmatsevtika faoliyati. Halol."},
    "TOSHMETALL":   {"nom": "Toshkent metallurgiya",  "holat": "🟢 HALOL",    "sabab": "Metall prokat va po'lat ishlab chiqarish. Halol."},
    "QIZILQUM":     {"nom": "Qizilqumsement",         "holat": "🟢 HALOL",    "sabab": "Qurilish materiallari va sement ishlab chiqarish. Halol."},
    "UZTELECOM":    {"nom": "O'ztelekom (Uztelecom)", "holat": "🟡 SHUBHALI", "sabab": "Aloqa xizmatlari. Kredit yuklamalari va aralash tushumlar sababli ehtiyotkorlik zarur."},
    "URGANG_XILE":  {"nom": "Urganch Ekskavator",     "holat": "🟢 HALOL",    "sabab": "Og'ir mashisasozlik va texnika ishlab chiqarish. Halol."},
    ...
    "IPOTEKABANK":  {"nom": "Ipoteka-bank",           "holat": "🔴 HAROM",    "sabab": "Ribo (foiz) asosidagi an'anaviy ipoteka va kredit xizmatlari."},
    "SQB":          {"nom": "O'zsanoatqurilishbank",  "holat": "🔴 HAROM",    "sabab": "Davlat an'anaviy banki — foizli operatsiyalar va ribo."},
    "XALKBANK":     {"nom": "Xalq Banki",             "holat": "🔴 HAROM",    "sabab": "An'anaviy bank tizimi va riboga asoslangan moliya faoliyati."},
    "ASAKABANK":    {"nom": "Asaka Bank",             "holat": "🔴 HAROM",    "sabab": "Ribo va an'anaviy foiz tizimidagi bank operatsiyalari."}
}
...
def main_menu():
    ...
    kb.add(
        ...
        types.KeyboardButton("💼 Mening Portfolio"),
...
def hisobla_rsi(closes, period=14):
    try:
        ...
        elif rsi <= 30: signal = "BUY 📈 (Oversold)"
...
def aksiya_tahlil(tiker):
    ...
    exchange = info.get('exchange', "Noma'lum")
    ...
    eps = info.get('trailingEps', "Noma'lum")
    fcf_raw = info.get('freeCashflow', None)
    fcf = format_raqam(fcf_raw) if fcf_raw else "Noma'lum"

    try:
        income = stock.financials.iloc[0, 0] if not stock.financials.empty else 0
        income_status = "📈 Foyda" if income > 0 else "📉 Zarar"
    except: income_status = "Noma'lum"
    ...
    def percent_safe(days):
        if uzunlik > days: return ((yopilish.iloc[-1] - yopilish.iloc[-(days + 1)]) / yopilish.iloc[-(days + 1)]) * 100
        return 0.0

    change_1d = percent_safe(1)
    change_1w = percent_safe(5)
    change_1m = percent_safe(22)
    change_1y = percent_safe(252) if uzunlik >= 252 else percent_safe(uzunlik - 1)
    ...
    if uzunlik > 60:
        ...
        fib_text = f"  • 🔵 38.2%: <code>{fib_382:.2f} {valyuta}</code>\n  • 🟡 50.0% (Equil): <code>{fib_500:.2f} {valyuta}</code>\n  • 🟠 61.8% (Golden): <code>{fib_618:.2f} {valyuta}</code>"
    else:
        fib_text = "  • Tarix yetarli emas."

    qarz = info.get('totalDebt', 0)
    nisbat = (qarz / market_cap_raw) * 100 if market_cap_raw else 0
    if nisbat < 30: halal = f"🟢 HALOL ({nisbat:.2f}%)"
    elif 30 <= nisbat <= 33: halal = f"🟡 SHUBHALI ({nisbat:.2f}%)"
    else: halal = f"🔴 HAROM ({nisbat:.2f}%)"
    ...
    try:
        ...
        if holders is not None and not holders.empty:
            for index, row in holders.head(3).iterrows():
                ...
                kitlar_text += f"  • 🏢 {html.escape(str(holder_name))}: <code>{format_raqam(shares_held)}</code> ta\n"
        else: kitlar_text = "  • Fondlar topilmadi.\n"
    except: kitlar_text = "  • Yuklashda cheklov.\n"
    ...
    if target_mean:
        ...
        target_text = f"  • 🎯 O'rtacha maqsad: <b>{target_mean} {valyuta}</b> ({potential_str})"
        ...
        try:
            ...
            if insider_trans and 'data' in insider_trans and insider_trans['data']:
                ...
                for t in trades:
                    ...
                    insider_lines.append(f"  • {html.escape(name[:15])}: <b>{action}</b> (<code>{abs(share_change):,}</code>)")
                insider_text = "\n".join(insider_lines)
        except: pass

    bugun_str = datetime.now().strftime("%Y-%m-%d")
    uch_kun_oldin_str = (datetime.now() - timedelta(days=3)).strftime("%Y-%m-%d")
    news_text = ""
    if finnhub_client:
        try:
            news = finnhub_client.company_news(tiker, _from=uch_kun_oldin_str, to=bugun_str)
            if news:
                news_text = "\n".join([f"  • {html.escape(n['headline'][:50])}...\n    🔗 {html.escape(n['url'])}" for n in news[:2] if 'headline' in n])
        except: pass
    if not news_text: news_text = "  • Yangiliklar e'lon qilinmadi."

    ball = 3.0
    if halal.startswith("🔴"): ball -= 1.5
    else: ball += 0.5
    if rsi >= 70: ball -= 1.5
    ...
    if halal.startswith("🔴"):
        ...
        taktika = "Kompaniya qarz yuklamasi to'g'ri kelmaydi."
    elif ball >= 4.5:
        maslahat_turi = "🔥 STRONG BUY 🟢"
        taktika = "Aksiya arzonlashgan, ko'rsatkichlari mustahkam."
    elif 3.5 <= ball < 4.5:
        maslahat_turi = "📈 ACCUMULATE 🟢"
        taktika = "Kompaniya barqaror. Bo'lib-bo'lib yig'ish mumkin."
    else:
        maslahat_turi = "HOLD (Kuzatish) 🟡"
        taktika = "Optimal koridorga kelguncha kutish afzal."

    izoh_punktlari = []
    if halal.startswith("🔴"): izoh_punktlari.append("• ⛔ Qarz yuklamasi yuqori (Ribo).")
    else: izoh_punktlari.append("• ✅ Qarz yuklamasi me'yorda (Halol).")
    if rsi >= 70: izoh_punktlari.append(f"• ⚠️ RSI ({rsi}): Aksiya juda qizigan.")
    ...
    if isinstance(pe, (int, float)) and pe > 35: izoh_punktlari.append(f"• 💸 P/E ({pe:.1f}): Kompaniya biroz qimmat.")
    bot_izohi = "\n".join(izoh_punktlari)

    javob = f"""
📊 <b>{tiker}</b> | {html.escape(info.get('longName', 'Noma\'lum'))}
 Sektor: {sektor} | ⚖️ Shariat: {halal}

━━━━━━━━━━━━━━━━━━━━
💰 <b>Narx ko'rsatkichlari:</b>
📌 Joriy narx: <b>{narx} {valyuta}</b>
📅 52W M/M: <code>{high_52w}</code> / <code>{low_52w}</code>
💎 Cap: {market_cap} | 💰 Div: <b>{div_yield}</b>

━━━━━━━━━━━━━━━━━━━━
📊 <b>Chuqur Fundamental Tahlil:</b>
📌 P/E Nisbati: {f'{pe:.2f}' if isinstance(pe, (int, float)) else pe} | {income_status}
📌 P/B: <b>{pb}</b> | ROE: <b>{roe}</b> | EPS: <b>{eps}</b>
📌 Erkin pul oqimi (FCF): <code>{fcf}</code>

━━━━━━━━━━━━━━━━━━━━
📐 <b>Fibonacci Darajalari (3M):</b>
{fib_text}

━━━━━━━━━━━━━━━━━━━━
📈 <b>Dinamika:</b> 1D: {change_1d:+.2f}% | 1W: {change_1w:+.2f}% | 1M: {change_1m:+.2f}%

━━━━━━━━━━━━━━━━━━━━
🧭 <b>Indikatorlar:</b>
📌 RSI (14): <code>{rsi}</code> → <b>{rsi_signal}</b>
📌 MACD: <b>{macd_signal}</b> | Bollinger: <b>{bb_signal}</b>

━━━━━━━━━━━━━━━━━━━━
🎯 <b>Wall Street Prognozi:</b>
{target_text}

━━━━━━━━━━━━━━━━━━━━
👥 <b>Insayderlar Harakati:</b>
{insider_text}
...
Baho: {ball:.1f}/5.0 {yulduzlar} -> <b>{maslahat_turi}</b>
📝 Izoh: <i>{bot_izohi}</i>
"""
    return javob, tiker
...
def handle_all(message):
    ...
    if text == "🔍 RSI Skriner (30/70)":
        ...
        kb.add(*[types.InlineKeyboardButton(t, callback_data=f"analyze_{t}") for t in ["AAPL", "TSLA", "NVDA", "MSFT", "JNJ", "MU"]])
        bot.send_message(message.chat.id, "👇 Tahlil uchun aksiyani tanlang:", reply_markup=kb)
        return
        
    elif text == "🔴 Harom aksiyalar":
        kb = types.InlineKeyboardMarkup(row_width=2)
        kb.add(*[types.InlineKeyboardButton(t, callback_data=f"analyze_{t}") for t in ["JPM", "BAC", "MCD", "NFLX"]])
        bot.send_message(message.chat.id, "👇 Tahlil uchun aksiyani tanlang:", reply_markup=kb)
        return
        
    elif text == "🟡 Shubhali aksiyalar":
        kb = types.InlineKeyboardMarkup(row_width=2)
        kb.add(*[types.InlineKeyboardButton(t, callback_data=f"analyze_{t}") for t in ["AMZN", "META", "V", "PYPL"]])
        bot.send_message(message.chat.id, "👇 Tahlil uchun aksiyani tanlang:", reply_markup=kb)
        return
        ...
        kb = types.InlineKeyboardMarkup(row_width=3)
        kb.add(*[types.InlineKeyboardButton(t, callback_data=f"analyze_{t}") for t in ["AAPL", "MSFT", "AMZN", "GOOGL"]])
        bot.send_message(message.chat.id, "🇺🇸 S&P 500 aksiyalari:", reply_markup=kb)
        return
    elif text == "🏛️ NASDAQ":
        kb = types.InlineKeyboardMarkup(row_width=3)
        kb.add(*[types.InlineKeyboardButton(t, callback_data=f"analyze_{t}") for t in ["NVDA", "TSLA", "META", "MU"]])
        bot.send_message(message.chat.id, "🏛️ NASDAQ aksiyalari:", reply_markup=kb)
        return
    elif text == "🏢 NYSE":
        kb = types.InlineKeyboardMarkup(row_width=3)
        kb.add(*[types.InlineKeyboardButton(t, callback_data=f"analyze_{t}") for t in ["NKE", "DIS", "KO", "WMT"]])
        bot.send_message(message.chat.id, "🏢 NYSE aksiyalari:", reply_markup=kb)
        return

    elif text == "💼 Mening Portfolio":
        bot.send_message(message.chat.id, "💼 Mening Portfolio bo'limi tez orada ishga tushadi.")
        return
