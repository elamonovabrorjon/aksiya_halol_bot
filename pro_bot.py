...
def start(message):
    ...
    markup.add("📊 Fond bozori", "₿ Crypto", "📊 Bookmap", "🆘 Adminlik (Yordam)")
    bot.send_message(message.chat.id, "Assalomu alaykum! Tahlil uchun tiker kiriting.", reply_markup=markup)

@bot.message_handler(func=lambda message: True)
def handle_message(message):
    if message.text in ["📊 Fond bozori", "₿ Crypto", "📊 Bookmap", "🆘 Adminlik (Yordam)"]:
        bot.reply_to(message, "Tiker kiriting (Masalan: AAPL, BTC):")
    else:
        bot.reply_to(message, get_18_point_analysis(message.text), parse_mode="HTML")

# Botni ishga tushirish (Render uchun eng barqaror usul)
